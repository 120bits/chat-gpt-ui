import * as AzureSDK from './AzureSDK.js';
...
window.playerAudioQueue = []
window.isPlaying = false
setInterval(() => {
  if (playerAudioQueue.length > 0 && !isPlaying) {
    isPlaying = true;
    console.log(playerAudioQueue)
    const audioUrl = playerAudioQueue[0];
    const audioElement = new Audio(audioUrl);
    // Add event listener for the "ended" event.
    audioElement.addEventListener('ended', function() {
      console.log('Audio playback finished!');
      playerAudioQueue.splice(0, 1);
      window.isPlaying = false;
      if (playerAudioQueue.length === 0) {
        CN_IS_READING = false;
        CN_SPEECHREC.start();
        CN_IS_LISTENING = true; // Ensure this flag is set to true here
      }
      // You can perform any actions you want here when the audio finishes playing.
    });
    // Play the audio.
    audioElement.play();
  }
...
function CN_SayOutLoud(text) {
  ...
  console.info('AZURE GEN: ' + text)
  AzureSDK.genAudio({
    text,
    key: "",
    region: "germanywestcentral",
    voice: "en-US-SaraNeural", // "nl-NL-FennaNeural",
    style: "neutral"
  }).then((audioURL) => {
    console.info('AZURE CALLBACK');
    if (audioURL) {
      window.playerAudioQueue.push(audioURL)
    }
  })
  return
...
function CN_CheckNewMessages() {
  // Any new messages?
  var currentMessageCount = jQuery('.c-message').length - 1;
  ...
  if (currentMessageCount > CN_MESSAGE_COUNT) {
    ...
    CN_CURRENT_MESSAGE = jQuery('.c-message').eq(-2).find('.c-message__markdown');
    CN_CURRENT_MESSAGE_SENTENCES = []; // Reset list of parts already spoken
    CN_CURRENT_MESSAGE_SENTENCES_NEXT_READ = 0;
  }

  // console.log(`Messages: ${currentMessageCount}`, CN_CURRENT_MESSAGE)

  // Split current message into parts
  if (CN_CURRENT_MESSAGE && CN_CURRENT_MESSAGE.length) {
    var currentText = jQuery('.c-message').eq(-2).find('.c-message__markdown p').text() + '';
    console.log("currentText:" + currentText);

    // Remove code blocks?
    // if (CN_IGNORE_CODE_BLOCKS) {
    //   currentText = jQuery('.c-message').eq(-2).find('.c-message__markdown').contents().not('pre').text();
...
function CN_SendMessage(text) {
  ...
  jQuery('.c-textarea-main').focus();
  var existingText = jQuery('.c-textarea-main').val();
  ...
  var height = rows * 24;
  jQuery('.c-textarea-main').css('height', height + 'px');

  // Send the message, if autosend is enabled
  if (CN_AUTO_SEND_AFTER_SPEAKING) {
    jQuery('.c-save-and-submit').click();
...
function CN_StartSpeechRecognition() {
  ...
  CN_SPEECHREC.onresult = (event) => {
    ...
    if (CN_RemovePunctuation(final_transcript) == CN_SAY_THIS_WORD_TO_STOP.toLowerCase().trim()) {
      ...
      // Click button
      jQuery('.c-textarea-main').closest('div').find('button').click();
...
function CN_SetTextareaValue(text) {
  const textarea = jQuery('.c-textarea-main')[0];
...
function CN_StartTTGPT() {
  ...
  setTimeout(function() {
    ...
    var currentMessageCount =  jQuery('.c-message').length;
    if (currentMessageCount > CN_MESSAGE_COUNT) {
      // New message!
      CN_MESSAGE_COUNT = currentMessageCount;
